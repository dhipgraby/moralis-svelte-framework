...
import { getWei } from "@helpers/main";
...
export async function getDragon(catId) {

    let contractData = await contractInfo()
                
    try {
        let dragon;
        await contractData.interfaces.dragon.methods.getDragon(catId).call({},
...
async function breedCats(mumId, dadId) {
    try {
        await contractInfo().interfaces.dragon.methods.breed(mumId, dadId).send({}, function (err, txHash) {
...
export async function isTokenOnSale(dragonId) {
    try {
        let isOnSale
        await Instance_Of_Marketplace.methods.isTokenOnSale(dragonId).call({}, function (errMsg, onSale) {
        ...
        console.log("Error from isCatOnSale(dragonId): " + error)
        console.log("Defaulting to returning false ... continuing")
        return false
    }
}

async function getForSaleDetails(dragonId) {
    ...
    try {
        await contractData.interfaces.marketplace.methods.setOffer(salePriceInWei, dragonId).send({}, function (err, txHash) {
            if (err) {
                ...
                setAlert(JSON.parse(err),txHash)
            }
        })
    }
    catch (err) {
