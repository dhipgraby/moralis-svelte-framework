...
export async function isTokenOnSale(dragonId) {
    ...
    try {
        let isOnSale
        await contractData.interfaces.marketplace.methods.isOnOffer(dragonId,OfferType.ForSale,TokenType.Dragon).call({}, function (err,onSale) {
            console.log(onSale)
            isOnSale = onSale
        })
        return isOnSale
...
export async function getForSaleDetails(dragonId) {
    ...
    let forSaleDetails;
    try {    
        await contractData.interfaces.marketplace.methods.getOffer(dragonId,OfferType.ForSale).call({}, function (err,offer) {      
            ...
            forSaleDetails = {
                ...
                priceInWei: offer.terms.sale.price,        
            }
        })
...
export async function setForSale(dragonId, salePriceInWei) {
    ...
    let saleTerms = {
        price: await getWei(salePriceInWei),
        rental: {
            deposit: 0,
            duration: 0,     
            returnEnergy: 0
