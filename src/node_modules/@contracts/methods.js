...
import { contractInfo,balances,updateBalances,contractAddress } from "@storage/contract";
...
export const fundContract = async (amount) => {
    ...
    try {
        ...
        alertTrigger.update(value =>
            value = alertParser('You fund the contract dapp with: ' + amount + ' eth', 'success')
        )
...
export const placeBet = async (face,amount) => {
    ...
    try {
       ...
       const { id, queryId }  = await event.events.playGame.returnValues
       ...
       alertTrigger.update(value =>
            value = alertParser(
                `You Bet ` + ((face == 1) ? `Hi!`: `Low!`)  + `
                <br>For amount of  <b>` + amount + ` ETH</b>
                <br> Game Id: <b>` + id + `</b>`, 'info')
        )
