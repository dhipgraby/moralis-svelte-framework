...
export const allFunctions = [ 
     {
         name:"createKittie",         
         method: async (dna) => await createKittie(dna),
         isCall:false,
         default:"8051882761572390"
     },
     {
        name:"getCatDetails",         
        method: async (catId) => await getCatDetails(catId),
        isCall:true,
        default:"1"
    },
    {
        name:"getAllYourCatIds",         
        method: async () => await getAllYourCatIds(),
        isCall:true,
        default:null
    },
    {        
...
export async function createKittie(dna) {
    try {
...
export async function getCatDetails(catId) {
    ...
    try {
        ...
        setAlert("function Error, try again.",'danger')
    }
}
...
async function breedCats(ids) {
    ...
    try {
        await contractData.interfaces.kittyContract.methods.breed(mumId, dadId).send({}, function (err, txHash) {
...
async function getAllCatIdsOnSale() {
    ...
    try {  
        await contractData.interfaces.marketplace.methods.getAllTokenOnSale().call({}, function (err, idsTokensOnSale) {
            ...
            output(idsTokensOnSale,"idsTokensOnSale")
            setAlert('Success' ,'success')               
        })
    }
    catch (error) {
