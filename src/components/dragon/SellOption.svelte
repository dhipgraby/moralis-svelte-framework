...
<BasicModal {...modalData} id={"dragonModal" + dragonProps.id}>
    ...
    {:else}      
        <div align="center">
            ...
            <button class="btn btn-danger text-light" data-bs-dismiss="modal" on:click={() => singleApproval(dragonProps.id) }>Approve single token</button>
            <br>
            <p><small> Approve marketplace for your tokens</small></p>
            <button class="btn btn-danger text-light" data-bs-dismiss="modal" on:click={() => setApprovalForAll() }>Approve for All</button>
        </div>
    {/if}
</BasicModal>
